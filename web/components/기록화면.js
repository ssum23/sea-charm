...
/* 숫자 앞에 붙는 말 — 「총 12마리를 판정했습니다」 / 「오늘 3마리를…」 */
function 기간앞말(기간) {
  var 칸 = 기간목록.find(function (g) { return g.값 === 기간; });
  if (!칸 || 칸.값 === '전체') return '총';
  return 칸.이름;
}
  ...
  return (
    <화면틀
      ...
      /* 🔴 2026-08-07 — 전에는 무엇을 눌러도 **전체 마릿수**가 떴다.
         ...
         이제 고른 기간의 마릿수를 보여주고, 앞말로 어느 기간인지 밝힌다. */
      /* 🔴 2026-08-13 — 머리를 두 줄로 (홈과 같은 모양).
         전에는 날짜 / 제목 / 큰 숫자 세 줄이었다 */
      요약={`${기간앞말(기간)} ${준비 ? 본것.length : 0}마리를 판정했습니다`.trim()}
