...
/* 🔴 2026-08-10 — 열쇠에 **띠**를 넣는다 (사장님 지적 「년도를 바꿔도 똑같다」).
 *
 * 전에는 물때만 열쇠로 썼다. 그래서 생년을 바꾸면 **띠와 띠말만** 바뀌고
 * 오늘의 자리·물고기 한마디·행운의 색·숫자는 그대로였다. 종이의 대부분이 안 바뀌니
 * 사람 눈에는 **같은 부적**으로 보였다.
 *
 * 이제 「물때 + 띠」가 한 장의 열쇠다.
 *   · 같은 사람이 같은 물때에 여러 번 열면 → **여전히 똑같은 한 장** (원칙 유지)
 *   · 띠를 바꾸면 → **다른 한 장** (사장님 결정 2026-08-10)
 * 🔴 부적 엔진(`app/부적.js`)은 한 글자도 안 건드린다. 화면이 열쇠만 다르게 준다. */
function 이번물때행운(오늘, 띠) {
  const 날 = 부적.물때키(오늘) + '|' + (띠 || '');
  try {
...
/* 🔴 2026-08-14 — 노랑을 청록으로 (사장님 선택 「1안」).
   까닭 — 앱 주색이 청록이 되면서 **부적만 노랗게 혼자 놀았다.**
   이 화면에만 색이 27개 직접 박혀 있었고(홈·물어보기는 0개) 그게 원인이었다.
   🔵 도장(大潮)만 금색을 남긴다 — 도장은 원래 금박이라 그게 맞다. */
const 종이 = {
  바탕: '#ffffff',
  글: '#2E4A50',
  진한글: '#00414D',
  강조: '#0098B2',          // 「오 늘 의 바 다」 같은 작은 글씨
  테: '#00414D',            // 굵은 테두리 · 물때 딱지 바탕
  그림자: '#7FD9E5',        // 노란 그림자 · 지느러미 · 도장 바탕
  물고기: '#7FCBE0',
  말풍선바탕: '#F1FAFC',
  말풍선글: '#00505E',
  말풍선테: '#9FD9E4',
...
export default function 부적화면() {
  ...
  useEffect(() => {
    if (!지금) return;
    /* 🔴 띠를 먼저 구해 행운의 열쇠에 넣는다 — 띠가 바뀌면 다른 부적이 나와야 한다 */
    const 이띠 = 생년 ? 부적.띠(Number(생년)) : null;
    const 몫 = 이번물때행운(지금, 이띠);
      ...
      {생년물어야함 ? (
        ...
        결과 && (
          ...
          <>
            ...
            <지난부적
              목록={이력.filter((x) => x.키 !== 결과.물때키 + '|' + (결과.띠 || ''))}
              펼침={이력펼침}
              펼치기={() => 이력펼침바꾸기(!이력펼침)}
            />
          </>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center',
      ...
      >
        <defs>
          ...
          {/* 🔴 2026-08-07 — 막대가 **구멍 위에서만** 보이게 자르는 틀.
              구멍은 y=82 에 있다. 그 아래로 내려간 부분은 통 속이므로 보이면 안 된다. */}
          <clipPath id="구멍위">
            <rect x="58" y="-160" width="44" height="242" />
          </clipPath>
        </defs>
        ...
        {/* 🔴 2026-08-07 — 막대를 **여기서** 그린다 (사장님 지적 「상자 뒤에서 나온다」).
            전에는 통보다 **먼저** 그려서, 막대가 넓은 윗판 뒤에서 스르륵 올라오는 것처럼
            보였다. 구멍은 작은데 나오는 자리는 판 전체였다 — 그래서 「뒤에서 나온다」로 읽혔다.
            이제 통을 다 그린 **뒤에** 막대를 얹고, `구멍위` 틀로 **구멍(y=82) 위쪽만** 남긴다.
            아래로 내려간 부분은 통 속이라 사라진다. 그러면 **구멍에서 솟는 것**으로 보인다.
        ...
        {/* 구멍의 **앞쪽 테두리** — 막대 위에 덮어 그린다.
            이게 있어야 막대가 구멍 「속으로」 들어가 있는 것으로 보인다 */}
        <path d="M70 82 A 10 4.6 0 0 0 90 82" fill="none"
              stroke="#2f1b08" strokeWidth="2.2" strokeLinecap="round" />
