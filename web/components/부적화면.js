...
export default function 부적화면() {
  ...
  useEffect(() => {
    if (!지금) return;
    결과바꾸기(부적.발급(생년 ? 생년 + '-01-01' : null, 지금, 이번물때행운(지금)));
  }, [생년, 지금]);
        ...
        결과 && (
          <>
            ...
            <Typography sx={{ fontSize: 크기.작게, color: 색.아주흐린글, lineHeight: 1.75, padding: '0 4px' }}>
              ...
              바뀔 때 새 부적이 나옵니다(보통 3~5일). 행운의 색과 숫자는 그 물때가 시작될 때
              뽑습니다(색 14가지 × 숫자 12가지 = 168가지). 다시 열어도 같은 것이 나와요.
              <br />
              물때는 달의 위상을 계산한 것이고, 나머지는 재미로 정해 둔 문구예요.{' '}
              <b style={{ color: 색.흐린글 }}>
                안 좋은 운세는 아예 넣지 않았고, 어획도 안전도 점치지 않습니다.
              </b>
            </Typography>
...
function 생년묻기({ 입력, 입력바꾸기, 확정, 건너뛰기 }) {
  ...
  return (
    ...
    >
      ...
      <FlexBox alignItems="baseline" justifyContent="center" gap={4} sx={{ padding: '6px 0 2px' }}>
        <Typography
          weight="bold"
          sx={{
            fontSize: 54,
            lineHeight: 1.1,
            letterSpacing: '-.02em',
            color: 입력 ? 색.글 : 색.아주흐린글,
          }}
        >
          {입력.padEnd(4, '0')}
        </Typography>
        <Typography weight="bold" sx={{ fontSize: 20, color: 색.흐린글 }}>
          년
        </Typography>
      </FlexBox>
