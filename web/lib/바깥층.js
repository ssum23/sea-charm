...
export function 곳들(한바다) {
  ...
  const 줄들 = [];
  if (한바다.수온 != null) {
    줄들.push({ 곳: 한바다.수온잰곳, 잰때: 한바다.수온잰때, 수온: 한바다.수온 });
  }
  if (한바다.파고 != null || 한바다.풍속 != null) {
    줄들.push({ 곳: 한바다.바다잰곳, 잰때: 한바다.바다잰때, 파고: 한바다.파고, 풍속: 한바다.풍속 });
  }
  return 줄들;
}
...
export function 볼만한곳(한곳) {
  if (!한곳) return false;
  const 수온살아 = 한곳.수온 != null && 쓸만한가(한곳.잰때, 12);
  const 바다살아 = (한곳.파고 != null || 한곳.풍속 != null) && 쓸만한가(한곳.잰때, 24);
  return 수온살아 || 바다살아;
}
