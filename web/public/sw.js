...
/* 🔴 2026-08-10 — 판 번호를 **사람이 적지 않는다.**
 *
 * 8월 7일에 'seacharm-v3' 로 적어둔 뒤 **한 번도 안 바뀌었다.**
 * 그동안 고친 것이 폰에서는 전부 **옛 저장분에 덮여** 보이지 않았다.
 * (「새로고침하면 되다가 또 옛날 것이 뜬다」 — 새 조각과 옛 조각이 섞인다)
 *
 * 주석에 「판을 바꿔야 한다」고 적어둔 것만으로는 못 막았다. **사람은 잊는다.**
 * 이제 빌드가 **만들어진 파일 목록으로 번호를 계산해** 채워 넣는다.
 * 코드가 한 글자라도 바뀌면 파일 이름이 바뀌고, 그러면 번호도 저절로 바뀐다. */
var 판 = 'seacharm-/*__판__*/';
