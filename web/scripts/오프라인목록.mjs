/* 오프라인에 미리 받아둘 목록을 자동으로 만든다.
 *
 * 왜 이게 필요한가 —
 * 앱이 바다에서 열리려면 「무엇을 미리 받아둘지」를 `sw.js` 가 알아야 한다.
 * 그런데 그 목록을 **사람이 손으로 적으면 반드시 낡는다** —
 * 화면을 하나 추가하거나 부품 이름이 바뀌면(빌드할 때마다 이름에 붙는 숫자가 달라진다)
 * 목록만 옛것으로 남고, 그 화면은 바다에서 안 열린다.
 * 그래서 **빌드가 끝난 폴더를 그대로 훑어** 목록을 채운다. 사람이 손댈 일이 없다.
 *
 * 언제 도는가 — `npm run build` 가 `next build` 다음에 자동으로 부른다.
 * 사람이 직접 실행할 일은 없다.
 *
 * 🔴 목록을 둘로 나누는 이유 —
 *   핵심(화면·부품·아이콘)이 없으면 앱이 아예 안 열린다 → 설치할 때 바로 받는다.
 *   글꼴 조각은 없어도 앱이 열린다(폰 기본 글꼴로 보인다) → 뒤에서 천천히 받는다.
 *   글꼴이 3MB라 이걸 같이 받으면 첫 방문이 눈에 띄게 느려진다.
 */

import fs from 'node:fs';
import crypto from 'node:crypto';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const 여기 = path.dirname(fileURLToPath(import.meta.url));
const 뿌리 = path.join(여기, '..');
const 나온곳 = path.join(뿌리, 'out');
const 밑 = process.env.NEXT_PUBLIC_BASE_PATH || '';

if (!fs.existsSync(나온곳)) {
  console.log('[오프라인] out 폴더가 없습니다 — 건너뜁니다.');
  process.exit(0);
}

/* 폴더를 통째로 훑는다 */
function 훑기(폴더, 담을곳 = []) {
  for (const 것 of fs.readdirSync(폴더, { withFileTypes: true })) {
    const 길 = path.join(폴더, 것.name);
    if (것.isDirectory()) 훑기(길, 담을곳);
    else 담을곳.push(path.relative(나온곳, 길).split(path.sep).join('/'));
  }
  return 담을곳;
}

/* 🔴 2026-08-10 — 판 번호를 **파일로도 남긴다**(`판.txt`).
 *
 * 왜 — 화면이 「지금 폰에 든 판」을 알아야 하는데, 저장 장치에 말을 걸어 물어보는 방식은
 * **아직 화면을 맡기 전이거나 옛 장치가 붙어 있으면 답이 안 온다.** 실제로 `판 ?` 이 떴다.
 * 파일로 두면 **미리 받아두는 목록에 같이 들어가므로**, 화면이 그 파일을 읽으면
 * **폰에 실제로 저장돼 있는 판**이 그대로 나온다. 말을 걸 필요가 없다. */
/* 🔴 이름을 `.txt` 로 두면 안 된다 — 아래 `뺄것` 이 `.txt` 를 **일부러 제외**한다
 *    (화면 사이 이동에 쓰는 부속 파일들이 `.txt` 라서). `.json` 으로 둔다. */
const 판자리 = path.join(나온곳, '판.json');
fs.writeFileSync(판자리, JSON.stringify({ 판: '자리채움' }));

const 전부 = 훑기(나온곳);

/* 미리 받지 않는 것 —
 *  sw.js       : 자기 자신
 *  *.txt       : 화면 사이 이동에 쓰는 부속 파일. 없으면 브라우저가 주소를 새로 연다(=동작한다)
 *  404.html    : 오프라인에서 쓸 일이 없다 */
const 뺄것 = (u) => u === 'sw.js' || u.endsWith('.txt') || u === '404.html';

/* 주소로 바꾼다.
 *  out/index.html        → {밑}/
 *  out/catch/index.html  → {밑}/catch/
 *  out/_next/…/abc.js    → {밑}/_next/…/abc.js
 * 🔴 `index.html` 을 그대로 적으면 안 된다 — 브라우저가 부르는 주소는 폴더 주소다. */
function 주소로(u) {
  if (u === 'index.html') return 밑 + '/';
  if (u.endsWith('/index.html')) return 밑 + '/' + u.slice(0, -'index.html'.length);
  return 밑 + '/' + u;
}

const 글꼴조각 = (u) => u.startsWith('font/woff2-dynamic-subset/');

const 핵심 = 전부.filter((u) => !뺄것(u) && !글꼴조각(u)).map(주소로).sort();
const 글꼴 = 전부.filter((u) => !뺄것(u) && 글꼴조각(u)).map(주소로).sort();

/* 🔴 첫 화면이 목록에 없으면 오프라인이 통째로 실패한다. 여기서 잡는다. */
if (!핵심.includes(밑 + '/')) {
  console.error('[오프라인] 🔴 첫 화면(' + 밑 + '/)이 목록에 없습니다. 빌드를 확인하세요.');
  process.exit(1);
}

const 적기 = (a) => a.map((u) => JSON.stringify(u)).join(',\n  ');

const 원본길 = path.join(나온곳, 'sw.js');
let sw = fs.readFileSync(원본길, 'utf8');

/* 표시를 찾지 못하면 조용히 넘어가면 안 된다 — 오프라인이 안 되는 채로 배포된다 */
for (const 표시 of ['/*__밑주소__*/', '/*__핵심__*/', '/*__글꼴__*/', '/*__판__*/']) {
  if (!sw.includes(표시)) {
    console.error('[오프라인] 🔴 sw.js 에서 ' + 표시 + ' 를 찾지 못했습니다.');
    process.exit(1);
  }
}

/* 🔴 저장분 판 번호 — 사람이 안 적는다.
 *
 * 파일 목록으로 번호를 만든다. Next 는 코드가 바뀌면 파일 이름에 붙는 숫자를 바꾸므로,
 * **코드가 한 글자라도 바뀌면 이 번호도 바뀐다.** 그러면 폰이 옛 저장분을 버리고 새로 받는다.
 * 반대로 아무것도 안 바뀌었으면 번호도 그대로라 쓸데없이 다시 받지 않는다.
 *
 * 전에는 `sw.js` 안에 'seacharm-v3' 처럼 손으로 적어뒀는데, 2026-08-07 이후
 * **아무도 안 바꿔서 사흘치 고친 것이 폰에 하나도 안 들어갔다.** 그 일을 다시 겪지 않는다. */
/* 🔴 빌드 때마다 달라지는 한 칸은 빼고 센다.
 *    Next 는 `_next/static/<빌드마다 다른 이름>/` 폴더를 만드는데, 그 이름은
 *    코드가 그대로여도 매번 달라진다. 그걸 그대로 세면 **아무것도 안 고쳐도 판이 바뀌어**
 *    폰이 4MB 를 다시 받는다. 배 위에서 그건 손해다.
 *    그 칸만 같은 말로 바꿔놓고 세면, **코드가 바뀔 때만** 판이 바뀐다.
 *    (파일 이름에 붙는 내용 숫자는 그대로 남으므로 코드 변화는 놓치지 않는다) */
const 셀것 = 핵심.map((u) => u.replace(/_next\/static\/[^/]+\//, '_next/static/BUILD/')).sort();
const 판번호 = crypto.createHash('sha1').update(셀것.join('|')).digest('hex').slice(0, 10);

sw = sw
  .replace('/*__밑주소__*/', 밑)
  .replace('/*__판__*/', 판번호)
  .replace('/*__핵심__*/', '\n  ' + 적기(핵심) + '\n')
  .replace('/*__글꼴__*/', '\n  ' + 적기(글꼴) + '\n');

fs.writeFileSync(원본길, sw);
/* 판.txt 에 진짜 번호를 적는다 (목록을 센 뒤라야 번호를 안다) */
/* 🔴 커밋 말도 같이 적는다 — 판 번호만으로는 무엇이 들어왔는지 사람이 못 알아본다.
   깃허브가 넘겨준 값이고, 손으로 돌릴 때는 비어 있다. 첫 줄만 쓰고 길면 자른다. */
/* 🔵 2026-08-13 — 기준표가 언제 것인지도 같이 내보낸다.
 *
 * 왜 필요한가 — 법이 바뀌어 우리가 기준표를 고쳐 올려도,
 * **사용자 폰은 옛 저장분을 갖고 있어서 안 받을 수 있다.**
 * 그러면 그 사람은 **낡은 기준으로 판정을 받는다.** 이 앱에서 가장 위험한 일이다.
 * 이제 앱이 인터넷이 있을 때 서버 쪽 판을 확인해서, 기준일이 다르면 알려준다.
 * 🔴 기준표를 저절로 바꾸지는 않는다 — 검산을 안 거친 기준표가 더 위험하다. 알려주기만 한다. */
let 기준일 = '';
try {
  const 기준표 = JSON.parse(fs.readFileSync(path.join(뿌리, '..', 'app', 'species.json'), 'utf8'));
  기준일 = String((기준표 && 기준표.메타 && 기준표.메타.기준일) || '');
} catch (e) {
  console.log('[오프라인] 기준일을 못 읽었습니다 — 빈 값으로 둡니다.');
}

const 커밋말 = String(process.env.COMMIT_MSG || '').split('\n')[0].trim().slice(0, 60);
const 커밋조각 = String(process.env.COMMIT_SHA || '').slice(0, 7);
fs.writeFileSync(
  판자리,
  JSON.stringify({ 판: 'seacharm-' + 판번호, 말: 커밋말, 조각: 커밋조각, 기준일: 기준일 }),
);

const 잰다 = (목록) =>
  목록.reduce((합, u) => {
    const 파일 = path.join(나온곳, u.slice(밑.length).replace(/\/$/, '/index.html'));
    try { return 합 + fs.statSync(파일).size; } catch { return 합; }
  }, 0);

const 메가 = (n) => (n / 1024 / 1024).toFixed(1) + 'MB';
console.log(
  '[오프라인] 미리 받을 것 — 핵심 ' + 핵심.length + '개(' + 메가(잰다(핵심)) + ')' +
  ' · 글꼴 ' + 글꼴.length + '개(' + 메가(잰다(글꼴)) + ')' +
  ' · 저장분 판 seacharm-' + 판번호 + (커밋말 ? ' · ' + 커밋말 : '')
);
